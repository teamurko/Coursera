  ...
  def top = new MainFrame {
    ...
    val suggestions: Observable[Try[List[String]]] = {
      searchTerms.concatRecovered(term => {
        val suggest: Future[List[String]] = WikipediaSuggest.wikipediaSuggestion(term)
        ObservableEx(suggest)
      })
    }
    ...
    val suggestionSubscription: Subscription =  suggestions.observeOn(eventScheduler) subscribe {
      x => {
        ...
        else status.text = x.failed.get.toString()
      }
    }
    ...
    val selections: Observable[String] = {
      button.clicks.map(_ => {
        val s = suggestionList.selection
        if (s.items.length == 0) Option.empty
        else Option(s.items.head)
      }).filter(_.isDefined).map(_.get)
    }

    // TO IMPLEMENT
    val pages: Observable[Try[String]] = {
      selections.concatRecovered(selection => {
        val page: Future[String] = WikipediaSuggest.wikipediaPage(selection)
        ObservableEx(page)
      })
    }
    ...
    val pageSubscription: Subscription = pages.observeOn(eventScheduler) subscribe {
      x => {
        if (x.isSuccess) editorpane.text = x.get
        else editorpane.text = x.failed.toString()
      }
    }
