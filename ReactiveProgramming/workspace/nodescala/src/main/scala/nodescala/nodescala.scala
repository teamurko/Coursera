...
trait NodeScala {
  ...
  private def respond(exchange: Exchange, token: CancellationToken, response: Response): Unit = {
    ...
    while (token.nonCancelled && !p.isCompleted) {
      Thread.sleep(10)
    }
  ...
  def start(relativePath: String)(handler: Request => Response): Subscription = {
    ...
    val sb = l.start
    val ct = Future.run() {ct => future {
      while (ct.nonCancelled) {
        val freq = l.nextRequest
        for (f <- freq) yield respond(f._2, ct, handler(f._1))
        Thread.sleep(1)
      }
    }}
    Subscription.apply(sb, ct)
  }
